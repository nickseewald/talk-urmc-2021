...
Slides are available at <slides.nickseewald.com/urmc>
...
- **Naltrexone** is known to diminish pleasurable effects of alcohol `r Citep(bib, "oslinTargetingTreatmentsAlcohol2006")`

- Response to naltrexone is heterogeneous (adherence, biology, social support, etc.) `r Citep(bib, "nahum-shaniSMARTDataAnalysis2017")`
...
- Heterogeneity necessitates a supportive intervention alongside naltrexone
  ...
  - **Combined behavioral intervention**: targets patient's motivation for change, emphasizes social support `r Citep(bib, "longabaughOriginsIssuesOptions2005")`
...
- **Proximal outcomes** are the near-term goals of the DTR; perhaps a mechanism by which we can achieve the distal outcome.
  - Short-term, we want to lower risk of relapse by reducing the _proportion of drinking and heavy-drinking days over 8 weeks after adaptation_ `r Citep(bib, "leiSMARTDesignBuilding2012")`
...
.pull-right[
  ...
  - Sample size is similar, with an extra weighting factor
]
...
print_bib_rmd(bib, start = 1, stop = 4, .opts = list(bib.style = "authoryear"))
```
...
```{r, results='asis', echo=FALSE, warning=FALSE}
print_bib_rmd(bib, start = 5, .opts = list(bib.style = "authoryear"))
```
