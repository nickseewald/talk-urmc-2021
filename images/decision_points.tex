...
%			\node [align=center,anchor=west] {\large \textbf{Stage 1}}; & &  \node [align=center,xshift=5mm] {\large \textbf{Stage 2}};\\[5mm]
			...
			& \node (blank2) [blank] {}; & & \node (stg2NR) [treatment,stg2tx] {behavioral intervention + \\ medical management + \\ naltrexone}; \\[1cm]
			\node (dp1) [fill=jhnavy,text=white] {treatment outset}; & & \node (dp2) [xshift = -15mm,fill=jhnavy,text=white] {week 8}; & \\
		};
...
%		\draw[dashed,lightgray] (time2.north west) --++(90:33cm);
		...
		\draw[trialarrow] (blank1.center) -- node (rlab) [rlabel] {Responders} (stg2R.west);
		\draw[trialarrow] (blank2.center) -- node[nrlabel] {Non-responders} (stg2NR.west);
		
		\draw [dashed, jhnavy] (dp1.north) -- (stg1.west);
		\draw [dashed, jhnavy] (dp2.north) -- (rlab.north);
